...
export function mergeMealPlans(basePlans: MealPlanWeek[], storedPlans: MealPlanWeek[]) {
  ...
  for (const storedPlan of storedPlans.map(normalizeStoredWeek)) {
    ...
    if (idIndex !== -1) {
      merged[idIndex] = storedPlan;
      continue;
    }

    const keyIndex = merged.findIndex((week) => week.key === storedPlan.key);
    if (keyIndex !== -1 && editableWeekKeys.includes(storedPlan.key)) {
      merged[keyIndex] = storedPlan;
      continue;
    }
...
export function useStoredMealPlans(basePlans: MealPlanWeek[] = mealPlanWeeks) {
  ...
  useEffect(() => {
    function refreshPlans() {
      setPlans(loadStoredMealPlans(basePlans));
    }
    ...
    return () => {
      ...
      window.removeEventListener(mealPlanStorageEvent, refreshPlans);
    };
  }, [basePlans]);
...
export function hydrateMealPlanWeek(week: MealPlanWeek, recipes: Recipe[]): HydratedMealPlanWeek {
  return {
    ...
    days: week.days
      ...
      .filter((day): day is HydratedMealPlanWeek["days"][number] => day !== null),
  };
}

export function getHydratedMealPlanByKey(
  plans: MealPlanWeek[],
  key: WeekKey,
  recipes: Recipe[],
) {
...
export function getRequiredRecipeType(day: DayName) {
  return freezerKitDays.has(day) ? "freezer-kit" : "regular";
}
...
function getPreviousRecipeIds(plans: MealPlanWeek[], targetWeek: MealPlanWeek, windowWeeks = 4) {
  return new Set(
    plans
      .filter((plan) => plan.startsOn < targetWeek.startsOn)
      .sort((a, b) => b.startsOn.localeCompare(a.startsOn))
      .slice(0, windowWeeks)
      .flatMap((plan) => plan.days.map((day) => day.recipeId)),
  );
}

function pickRecipe(candidates: Recipe[], excludedIds: Set<string>) {
  const preferred = candidates.filter((recipe) => !excludedIds.has(recipe.id));
...
export function generateMealPlanWeek(
  plans: MealPlanWeek[],
  targetKey: Extract<WeekKey, "current" | "next">,
  recipes: Recipe[],
) {
  const targetWeek = getMealPlanByKey(plans, targetKey);
  const recentRecipeIds = getPreviousRecipeIds(plans, targetWeek);
  const usedRecipeIds = new Set(recentRecipeIds);

  return {
    ...targetWeek,
    label: weekLabels[targetKey],
    days: targetWeek.days.map((day) => {
      if (day.locked) {
        usedRecipeIds.add(day.recipeId);
        return day;
      }

      const candidates = getCandidateRecipesForDay(day.day, recipes);
      const recipe = pickRecipe(candidates, usedRecipeIds);
...
export function rerollMealPlanDay(
  plans: MealPlanWeek[],
  targetKey: WeekKey,
  dayName: DayName,
  recipes: Recipe[],
  ...
  const targetWeek = getMealPlanByKey(plans, targetKey);
  const recentRecipeIds = getPreviousRecipeIds(plans, targetWeek);
  const currentWeekRecipeIds = new Set(
    targetWeek.days
      .filter((day) => day.day !== dayName)
      .map((day) => day.recipeId),
  );
  const excludedIds = new Set([...recentRecipeIds, ...currentWeekRecipeIds]);
  const candidates = getCandidateRecipesForDay(dayName, recipes);
  const recipe = pickRecipe(candidates, excludedIds);

  if (!recipe) return targetWeek;

  return replaceMealPlanDay(targetWeek, dayName, recipe.id);
}

export function replaceMealPlanDay(week: MealPlanWeek, dayName: DayName, recipeId: string) {
  return {
    ...week,
    days: week.days.map((day) => (day.day === dayName ? { ...day, recipeId } : day)),
  };
}
...
export function setMealPlanDayLocked(week: MealPlanWeek, dayName: DayName, locked: boolean) {
  return {
    ...week,
    days: week.days.map((day) => (day.day === dayName ? { ...day, locked } : day)),
  };
}

export function upsertMealPlan(plans: MealPlanWeek[], nextWeek: MealPlanWeek) {
  const existingIndex = plans.findIndex((week) => week.id === nextWeek.id || week.key === nextWeek.key);
  if (existingIndex === -1) {
    return mergeMealPlans(plans, [nextWeek]);
  }

  return plans.map((week, index) => (index === existingIndex ? nextWeek : week));
}
